import type { AutoTradeConfig, AutoTradeWallet } from '../../shared/types/settings.types';
...
export class AutoTradeSimulator {
  ...
  private static readonly TICK_MS = AutoTradeSimulator.DEMO_MODE ? 3_000 : 20_000;

  /** Số ms tương ứng với 1 "phút" trong cấu hình interval_minutes. */
  private static readonly MINUTE_MS = AutoTradeSimulator.DEMO_MODE ? 2_000 : 60_000;
  ...
  public start(): void {
    ...
    void this.tick();
    this.timer = setInterval(() => { void this.tick(); }, AutoTradeSimulator.TICK_MS);
  }
  ...
  private async tick(): Promise<void> {
    try {
      ...
      const autoTrade: Partial<Record<AutoTradeWallet, AutoTradeConfig>> = settings?.autoTrade || {};
      const now = Date.now();
      ...
      for (const wallet of WALLETS) {
        ...
        const intervalMs = Math.max(1, cfg.interval_minutes ?? 60) * AutoTradeSimulator.MINUTE_MS;
